...
def send_scan_to_sylius(scan_id):
    sylius_scan_data = {"scan_id": scan_id}
    url = build_url()
    req = urllib.request.Request(url)
    req.add_header("Content-Type", "application/json")
    response = urllib.request.urlopen(req, json.dumps(sylius_scan_data).encode("utf-8"))
    return response
